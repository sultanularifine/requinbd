    ...
    <div class="blog-content">
        ...
        <article class="article">
            ...
            @if(isset($blog->images[0]))
                <div class="gallery"><img src="{{ asset($blog->images[0]->image) }}" alt="Image 1"></div>
            @endif
            
            @if($blog->description1)
                <div class="desc-section">{!! $blog->description1 !!}</div>
            @endif

            @if(isset($blog->images[1]))
                <div class="gallery"><img src="{{ asset($blog->images[1]->image) }}" alt="Image 2"></div>
            @endif

            @if($blog->description2)
                <div class="desc-section">{!! $blog->description2 !!}</div>
            @endif

            @if(isset($blog->images[2]))
                <div class="gallery"><img src="{{ asset($blog->images[2]->image) }}" alt="Image 3"></div>
            @endif
            @if($blog->description3)
                <div class="desc-section">{!! $blog->description3 !!}</div>
            @endif

            @if($blog->images && $blog->images->count() > 3)
                <div class="gallery">
                    @foreach($blog->images->slice(3) as $extra_image)
                        <img src="{{ asset($extra_image->image) }}" alt="Additional Image">
                    @endforeach
                </div>
            @endif
        </article>
